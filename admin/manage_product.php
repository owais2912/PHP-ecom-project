...
if(isset($_POST['submit'])){
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			...
			$image=$_FILES['image']['name'];
			move_uploaded_file($_FILES['image']['tmp_name'],PRODUCT_IMAGE_SERVER_PATH.$image);
			mysqli_query($con,"insert into product(categories_id,name,mrp,price,qty,short_desc,description,status,image,best_seller,sub_categories_id) values('$categories_id','$name','$mrp','$price','$qty','$short_desc','$description',1,'$image','$best_seller','$sub_categories_id')");
		}
		header('location:product.php');
		die();
	}
